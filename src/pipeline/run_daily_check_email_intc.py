...
def check_for_new_signals() -> dict:
    """检查是否有新的交易信号 (INTC日度策略)"""
    signal_file = project_root / "INTC" / "backtest_results" / "daily" / "signals_daily.csv"
...
def get_current_position(bars: list) -> dict:
    ...
    Returns:
        dict: 持仓信息
    """
    trades_file = project_root / "INTC" / "backtest_results" / "daily" / "trades_daily.csv"
...
def run_daily_check_with_email():
    ...
    try:
        ...
        try:
            ...
            realtime_quote = quotes_mgr.get_realtime_quote('INTC')
        ...
        try:
            ...
            health = fundamentals_mgr.calculate_financial_health('INTC')
        ...
        try:
            ...
            news_summary = news_mgr.get_news_summary('INTC', days=7)
        ...
        position_info = get_real_position('INTC', current_price)
        ...
        if signal_info['has_signal']:
            ...
            if fundamentals_mgr and health:
                print("  📊 基本面检查:")
                decision = fundamentals_mgr.should_allow_buy('INTC', min_score=40)
            ...
            email_service.send_signal_alert(
                ...
                strategy_name="INTC日度策略 (技术面+基本面+新闻情绪)"
            )
        else:
            print("✓ 暂无新交易信号")
            print()
            ...
            if market_env:
                if additional_info is None:
                    ...
                    additional_info += "\n\n🌍 市场环境快照:\n"
                additional_info += f"- 宏观环境: {market_env['macro']['environment']} ({market_env['macro']['risk_level']} risk)\n"
                additional_info += f"- 市场情绪: {market_env['sentiment']['overall_sentiment']} ({market_env['sentiment']['overall_score']:.1f}/100)\n"
            ...
            if health and health['score'] > 0:
                ...
                if health['details'].get('pe') != 'N/A':
                    additional_info += f"- PE比率: {health['details']['pe']:.2f}\n"
                if health['details'].get('roe') != 'N/A':
